...
def detect_query_domain(query: str, content_samples: list[str]) -> str:
    ...
    Args:
        ...
        content_samples: Sample content from search results
    ...
    if any(
        word in query_lower
        for word in ["gpu", "cuda", "reranking", "embedding", "model", "architecture", "docker", "api"]
    ):
        return "technical"
...
class Qwen3Reranker:
    ...
    def _load_model(self) -> bool:
        ...
        try:
            ...
            # Load model
            self.model = AutoModelForCausalLM.from_pretrained(self.model_name, **model_kwargs).eval()
